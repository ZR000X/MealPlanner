...
class Meal:
    ...
    def cost(self, prices, return_missing= False):
        ...
        if type(prices) is ProductRegistry:
            ...
            # ingr = "-------------------------\n "
            # keyerrors = ""
            # for name, portion in self.ingredients.items(): # key is item name, value is item unit
            #     try:            
            #         ingr += "Ingredient: "+name+"\n "
            #         ingr += "portion: "+str(portion)+"\n "
            #         ingr += "price: "+str(prices[name])+"\n "
            #         ingr += "cost per serving: "+str(portion * prices[name] / self.servings) +"\n"
            #         ingr += "-------------------------\n "
            #         cost += portion * prices[name ] # should be units of rands
            #     except KeyError: keyerrors += "Missing ingredient '"+name+"' in price registry. \n "
            # return [cost / (self.servings * U.serving), ingr, keyerrors]
        if return_missing:
